...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"deam": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + deam.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + deam.blockNumber + " (" + new Date(1000 * deam.getBlock(deam.blockNumber).timestamp) + ")\n";
		try {
